...
- Pull the published image:  
  ...
  ```
- Pull before running (if you haven’t built locally):  
  ```powershell
  docker pull shreyash42/data-lab:latest
  ```
...
- `run-standalone-interactive.sh` (bash): Prompts for container name and any number of extra host↔container bind mounts; default ports are always mapped. Uses `shreyash42/data-lab:latest` unless `IMAGE` is set. Pass `EXTRA_PORTS` if you want additional ports.
- `run-standalone-interactive.ps1` (PowerShell): Prompts for container name and any number of extra host↔container bind mounts; default ports are always mapped. Uses `shreyash42/data-lab:latest` unless `-Image` is provided. Pass `-ExtraPorts` if you want additional ports. Uses the repo root (parent of `scripts/`) for default mounts.
