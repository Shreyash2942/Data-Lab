...
```bash
bash ~/hive/scripts/hive_access_guide.sh
```

This prompts for username/password and prints:
- Dynamic host-side Hive connection values
- JDBC URLs for IDE tools (DBeaver/DataGrip/SQL tools)
- CLI connection examples (`hivecli`, `beeline`)
- Related Hadoop UI endpoints
...
- `hivelegacy` - wraps `~/app/tech/hive/legacy_cli.sh`, auto-connects to HS2, and shows prompts like `hive (default)>`.
- `hivecli` - wraps `~/app/tech/hive/cli.sh`, launching Beeline against the HTTP HS2 endpoint (default `localhost:10001/cliservice`, `auth=noSasl`, user `datalab`).
...
That launches HS2 over HTTP (`localhost:10001/cliservice`), waits for the port, and runs a verification query via the Beeline helper. Stop it with `bash ~/app/tech/hive/hs2.sh stop`. Connect via:
