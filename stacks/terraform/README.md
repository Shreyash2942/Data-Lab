...
Terraform CLI 1.7 ships in the container for quick infrastructure demos. Project files live under `~/terraform` (mirrors `repo_root/stacks/terraform`) and store their working state in `~/runtime/terraform`.

## Layout

| Path | Purpose |
| --- | --- |
| `stacks/terraform/main.tf` | Sample configuration (modify or replace with your own). |
| `stacks/terraform/variables.tf`, `stacks/terraform/outputs.tf` | Optional helpers depending on your setup. |
...
- Never commit `datalabcontainer/runtime/terraform` - it is already ignored; remove it to reset the demo quickly.
