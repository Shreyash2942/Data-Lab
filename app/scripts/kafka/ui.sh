...
ui::pid_alive() {
  [[ -f "${KAFDROP_PID_FILE}" ]] && kill -0 "$(cat "${KAFDROP_PID_FILE}")" 2>/dev/null
}
...
ui::start() {
  ...
  if ui::pid_alive; then
    ...
    echo "    URL: http://localhost:${KAFDROP_PORT}"
    return 0
  fi
...
EOF
  ...
  echo "[*] Starting Kafdrop on http://localhost:${KAFDROP_PORT} (logs: ${KAFDROP_LOG_FILE})..."
  JAVA_PROPS=(
...
EOF
  ...
  echo "[+] Kafdrop started (PID $(cat "${KAFDROP_PID_FILE}")) -> http://localhost:${KAFDROP_PORT}"
}
...
ui::status() {
  ui::cleanup_stale_pid
  if ui::pid_alive; then
    echo "[+] Kafdrop running (PID $(cat "${KAFDROP_PID_FILE}")) on http://localhost:${KAFDROP_PORT}"
  else
    echo "[-] Kafdrop not running."
  fi
}
