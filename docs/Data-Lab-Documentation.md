...
- `dbt/profiles.yml` is committed and points to a DuckDB database at `~/dbt/data_lab.duckdb`, so `dbt debug` works immediately.
...
- HDFS data directories live under `~/hadoop/dfs`, so they persist outside the container.

## Spark Master + Worker

- `dev/spark/conf/spark-env.sh` and `spark-defaults.conf` configure a localhost master (`spark://localhost:7077`), worker, and history server with logs under `~/spark/events`.
...
- `dev/hive/conf/hive-site.xml` configures an embedded Derby metastore stored in `~/hive/metastore_db` with a warehouse path at `~/hive/warehouse`.
...
- **Hudi (menu option 12)**: `~/app/services_demo.sh` runs `spark/hudi_example.py`, writing demo data to `~/hudi_tables`.
- **Iceberg (menu option 13)**: the same demo helper runs `spark/iceberg_example.py`, targeting a Hadoop catalog at `~/iceberg_warehouse`.
- **Delta Lake (menu option 14)**: the demo helper runs `spark/delta_example.py`, writing to `~/delta_tables`.

## Kafka Broker

- `dev/kafka/conf/server.properties` and `zookeeper.properties` define a single-node Kafka + Zookeeper stack with logs/data under `~/kafka`.
